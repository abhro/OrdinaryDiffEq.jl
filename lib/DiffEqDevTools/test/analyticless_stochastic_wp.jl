...
setups = [
    Dict(:alg => SRIW1())
    Dict(
        :alg => EM(), :dts => 1.0 ./ 5.0 .^ ((1:length(reltols)) .+ 1),
        :adaptive => false
    )
    Dict(
...
prob2 = SDEProblem(
    (du, u, p, t) -> prob.f(du, u, p, t), prob.g, prob.u0, (0.0, 0.1),
    prob.p
)
test_dt = 1 / 10^4
appxsol_setup = Dict(:alg => SRIW1(), :abstol => 1.0e-4, :reltol => 1.0e-4)
